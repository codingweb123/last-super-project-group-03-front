...
const parsePhone = (phone: string): string => {
	const match: string[] | null = phone.match(/\+38(0\d{2})(\d{3})(\d{2})(\d{2})/); // ["+380_________", "+38", "0__", "___", "__", "__"] or null.
	// If match isn't null,
...
export default function UserInfoForm() {
	...
	const handleInputNumber = (event: React.KeyboardEvent<HTMLInputElement>): void => {
		const value: string = event.currentTarget.value;
		// Leaves only digits.
		const digits: string = value.replace(/\D/g, "");
		const match: string[] | null = digits.match(/(\d{0,2}0)(\d{0,2})(\d{0,3})(\d{0,2})(\d{0,2})/); // might be from ["__0", "__0"] to ["__0_________", "__0", "__", "___", "__", "__"] or null.
